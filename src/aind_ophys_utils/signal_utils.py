    ...
    if size > 20 and len(input) > 200:
        return pd.Series(np.concatenate(
            (input[:size//2][::-1], input, input[:-size//2-1:-1]))
                        ).rolling(size, center=True).quantile(
            percentile/100).to_numpy(dtype)[size//2:-size//2]
    else:
...
def nanmedian_filter(
    ...
    filtered_trace: ndarray
    """
    filtered_trace = pd.Series(np.concatenate((input[:size//2][::-1], input, input[:-size//2-1:-1]))
                              ).rolling(size, center=True, min_periods=1).median().to_numpy(
        input.dtype if dtype is None else dtype)[size//2:-size//2]
...
def noise_std(
    ...
    max_num_samples: int
        Number of samples used for computing the noise when using method 'fft' or 'welch'.
    noise_range: tuple (float, float) between 0 and 0.5, default (.25, .5)
        Range of frequencies compared to Nyquist rate over which the PSD is averaged. 
    filter_length: int
        Length of the median filter to compute the rolling median-filtered signal,
        which is subtracted from the input `x` for ``method='mad'``
    axis: int
    ...
    if method == 'mad':
        if x.ndim > 1:
            ...
            if n_jobs == 1:
                return np.reshape([noise_std(y, method='mad', filter_length=filter_length)
                                  for y in x.reshape(-1, T)], dims).astype(x.dtype)
            else:
                res = ThreadPool(n_jobs).map(
                    lambda y: noise_std(y, method='mad', filter_length=filter_length), x.reshape(-1, T))
        ...
        if T > max_num_samples:
            x = np.concatenate((x[..., :max_num_samples // 3],
                                x[..., int(T // 2 - max_num_samples / 6):
                                  int(T // 2 + max_num_samples / 6)],
                                x[..., -max_num_samples // 3:]), axis=-1)
            T = x.shape[-1]
        if method == 'welch':
            ...
            xdft = xdft[..., slice(*(int(n/.5*len(xdft)) for n in noise_range))]
            psd = abs(xdft)**2 / T
