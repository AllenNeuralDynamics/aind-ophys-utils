...
@pytest.mark.parametrize(
    ...
    [
        ...
        (np.arange(1000.), 100, 21, list(range(10,1000)) + [999]*10),
        (np.arange(1000.), 50, 21, [5]*5 + list(range(5,995)) + [994]*5),
        (np.arange(1000.), 0, 21, [0]*10 + list(range(990))),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (np.arange(1000.), 21, [5]*5 + list(range(5,995)) + [994]*5),
    ],
)
...
@pytest.mark.parametrize(
    "x, expected, method",
    list(map(lambda x: list(chain(*x)), product(
    [[np.array([0, 1, 2, 3, np.nan]), np.nan],  # Has NaN
     [np.random.randn(20, 10000), [1]*20],      # just noise
     [np.random.randn(20, 10000) +              # Typical: noise+signal
      np.sin(np.linspace(0,100,200000).reshape(20,10000)), [1]*20]],
    [['welch'], ['mad'], ['fft']]))))
